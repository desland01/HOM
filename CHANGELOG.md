...
### Changed
- **Sticky Features Styling (sticky-features.tsx, tailwind.config.ts):** Refined the custom `animate-ember` slow-breathing glow effect on the "See Tier Details +" buttons to be much more subtle. The blur and scale were significantly reduced so the glow hugs tightly to the border, providing a premium, understated warmth rather than a wide aura.
